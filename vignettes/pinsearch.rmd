...
ps1 <-
    pinSearch(mod,
        ...
        test = "mod"  # use modification indices for search (default)
    )
```
...
Another possibility is to use the `pin_es()` function, which also works for partial invariance model not obtained from `pinSearch()`.
...
TODO: Need to create a documentation on

- pinsearch proceeds by types of invariant constraints (loadings, intercepts, residuals, etc)
- intercepts are freed when the corresponding loadings are freed.
- residual variances do not depend on loadings and intercepts constraints
- On residual covariances. 
...
One can also use the score test instead of the modification indices, as [starting in version 0.6-17, `lavaan` recommends using that for equality constraints](https://lavaan.ugent.be/history/dot6.html#version-0.6-17). This can be achieved with `test = "score"`.
